...
public class VideoQueue {
    ...
    public void processPlayerRequest(VideoInfo videoInfo, Player player) {
        cinemaModPlugin.getVideoStorage().saveVideoInfo(videoInfo).thenRun(() -> {
            ...
            switch (queueResult) {
                case QUEUE_FULL:
                    player.sendMessage(ChatColor.RED + "Очередь в этот кинотеатр переполнена.");
                    break;
                case ALREADY_IN_QUEUE:
                    player.sendMessage(ChatColor.RED + "Это видео уже находится в очереди.");
                    break;
                case QUEUE_LOCKED:
                    player.sendMessage(ChatColor.RED + "Владелец этого кинотеатра заблокировал очередь.");
                    break;
                case SUCCESSFUL:
